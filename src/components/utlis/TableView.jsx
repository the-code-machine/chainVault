...
export default function TableView() {
  ...
  const logDocs = async (tokenId, action) => {
    ...
    try {
      await contract.methods
        .logDocumentAction(tokenId, action)
        .send({ from: account });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const authors = await Promise.all(
        tokenIds.map((tokenId) => fetchAuthors(tokenId))
      );
      const uris = await Promise.all(
        tokenIds.map((tokenId) => fetchUri(tokenId))
      );
  ...
  async function handleOpen(uri, tokenId) {
    ...
    window.location.href =
      "https://lavender-certain-yak-875.mypinata.cloud/ipfs/" + uri;
    setLoading(false);
  }
            ...
            {documents.map((index, item) => (
              <>
                <tr key={item + 1}>
                  ...
                  <td class="px-3 py-4 whitespace-nowrap">
                    <button class=" inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 px-3 py-1.5">
                      Change Permissions
                    </button>
                  </td>
                  ...
                  <td class="px-3 py-4 whitespace-nowrap">
                    <span
                      onClick={() => handleOpenLogs(index.tokenId)}
                  ...
                  <td class="px-3 py-4 whitespace-nowrap">
                    <button
                      onClick={() => handleOpen(index.tokenUri, index.tokenId)}
                      class="px-4 py-1.5 inline-flex text-md leading-5 font-semibold rounded-full bg-primary text-white"
                    >
                      View Doument
                    </button>
                  </td>
...
const Logs = ({ logs, setOpen }) => {
  ...
  return (
    <div
      ...
      className=" fixed top-0 left-0 z-[1000000000] bg-white   bg-opacity-80 h-screen w-full flex justify-center items-center"
    >
